#!/usr/bin/env python3
"""
GR-EAT 체험단 운영 툴 - Firebase Functions
"""
...
# 데이터베이스 경로 설정 (Firebase Functions에서는 /tmp 사용)
...
# 임시 인스타그램 스크래핑 함수 (Firebase Functions에서는 Selenium 제한으로 Mock 데이터 사용)
def scrape_instagram_profile_mock(username):
    """Mock 인스타그램 프로필 스크래핑 (Firebase Functions 환경 제한으로 인한 임시 구현)"""
...
def api_applicants():
    """지원자 API"""
    # 데이터베이스 초기화 (Firebase Functions에서는 매 요청마다 초기화 필요할 수 있음)
    init_db()
...
def api(req: https_fn.Request) -> https_fn.Response:
    """Firebase Functions 래퍼 - Flask 앱을 감쌉니다"""
